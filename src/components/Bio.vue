...
$strings: (
  'I come from Italy, and I live in Munich since 2018!'
    'I developed my first website with plain html and css and a my first js lines around 2008 😱'
    'I work as professional web developer from 6 years ☕'
);
      ...
      @if $j < str-length(nth($strings, $i)) * 2 {
        ...
        #{percent($i, $j+1, -0.001)} {
          @if $j <= str-length(nth($strings, $i)) {
            ...
            content: quote(
              #{str_slice(nth($strings, $i), 1, str-length(nth($strings, $i)) - ($j - str-length(nth($strings, $i))))}
            );
          }
        }
      } @else {
...
div#bio-container {
  ...
  background-image: radial-gradient(rgba(#fff, 0.125), rgba(#fff, 0)),
